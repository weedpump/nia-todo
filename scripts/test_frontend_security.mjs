...
assert(serviceWorkerUpdatesSource.includes('ensureOfflineServiceWorkerReadyAfterHardReload') && serviceWorkerUpdatesSource.includes("register('/sw.js', { updateViaCache: 'none' })") && serviceWorkerUpdatesSource.includes("postMessage({ action: 'skipWaiting' })"), 'login/sidebar hard reload must reinstall an active offline service worker before navigation');
