...
assert(forceReloadAppSource.includes('canReachCurrentAppShell') && forceReloadAppSource.indexOf('if (!(await canReachCurrentAppShell()))') < forceReloadAppSource.indexOf('try {'), 'login/sidebar force reload must return before cleanup/reload while the app shell is unreachable');
...
assert(serviceWorkerUpdatesSource.includes('canReachCurrentAppShell') && serviceWorkerUpdatesSource.includes('hard-reload-online-check') && serviceWorkerUpdatesSource.indexOf('if (!(await canReachCurrentAppShell()))') < serviceWorkerUpdatesSource.indexOf('registration.unregister()'), 'login/sidebar force reload must prove the app shell is reachable before unregistering service workers or deleting caches');
