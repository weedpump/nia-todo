...
assert(!androidReminderSource.includes('.addAction(') && !androidReminderSource.includes('ACTION_MARK_DONE'), 'Android reminder notifications must not expose a completed action');
...
const connectionStatusSource = readFileSync(new URL('../web/static/js/features/connection-status.js', import.meta.url), 'utf8');
...
assert(forceReloadAppSource.includes('navigator.onLine === false') && forceReloadAppSource.includes('button.disabled') && forceReloadAppSource.includes("aria-disabled') === 'true") && forceReloadAppSource.indexOf('navigator.onLine === false') < forceReloadAppSource.indexOf('try {'), 'login/sidebar force reload must return before cleanup/reload while the app is offline or the reload button is disabled');
assert(serviceWorkerUpdatesSource.includes('refreshActiveServiceWorkerAppCache') && serviceWorkerUpdatesSource.includes("postMessage({ action: 'refreshAppCache' }") && serviceWorkerUpdatesSource.includes('hasActiveNiaTodoWorker'), 'login/sidebar force reload must preserve active nia-todo service workers and refresh their app cache in place for iOS PWA offline launch');
assert(serviceWorkerUpdatesSource.includes('navigator.serviceWorker.getRegistrations') && serviceWorkerUpdatesSource.includes('isNiaTodoServiceWorkerRegistration') && serviceWorkerUpdatesSource.includes('registration.unregister()'), 'login/sidebar force reload may unregister stale nia-todo service workers only when no active worker is available');
assert(serviceWorkerUpdatesSource.includes('caches.keys()') && serviceWorkerUpdatesSource.includes('isNiaTodoCacheName') && serviceWorkerUpdatesSource.includes('caches.delete(name)') && serviceWorkerUpdatesSource.includes('!hasActiveNiaTodoWorker'), 'login/sidebar force reload must only clear nia-todo CacheStorage when no active service worker can be preserved');
...
assert(downloadsSource.includes("linux: {") && downloadsSource.includes("filenamePrefix: 'nia-todo-desktop-v'") && downloadsSource.includes("filenameSuffix: '-linux-amd64.deb'") && downloadsSource.includes("return ['windows', 'android', 'linux']"), 'app downloads must expose Debian desktop packages from app-downloads manifests');
...
assert(downloadsSource.includes("app-download-icon-linux") && downloadsSource.includes("Debian-Paket herunterladen"), 'Debian downloads must have platform-specific UI labels');
