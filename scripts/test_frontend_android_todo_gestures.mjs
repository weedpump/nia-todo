...
async function run() {
  ...
  if (!dragDropSource.includes('NATIVE_AUTO_SCROLL_EDGE_PX') || !dragDropSource.includes('NATIVE_AUTO_SCROLL_TOP_EDGE_PX') || !dragDropSource.includes('nativeScrollContainer()') || !dragDropSource.includes('container.scrollTop += deltaY') || !dragDropSource.includes('scheduleNativeAutoScroll()')) {
    throw new Error('Native pointer drag must auto-scroll the app scroll container near viewport edges on Android, with an earlier top edge trigger');
  }
