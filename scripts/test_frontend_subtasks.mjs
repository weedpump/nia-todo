...
const TODO_TITLE = `Frontend subtasks persistence ${process.pid}-${Date.now()}`;

await withFreshDb(async () => {
  const { browser, page, loginApp, openTodoModal, waitForText, assertNoFrontendErrors } = await launchPage();
  try {
    await loginApp();
    await page.evaluate(title => localStorage.setItem('__subtask_test_title', title), TODO_TITLE);
    ...
    await page.waitForFunction(async () => {
      ...
      const todo = data.todos?.find?.(item => item.title === localStorage.getItem('__subtask_test_title'));
    ...
    const beforeReload = await page.evaluate(async () => {
      const token = localStorage.getItem('jwt_token');
      const data = await fetch('/api/todos', { headers: { Authorization: `Bearer ${token}` }, credentials: 'include' }).then(r => r.json());
      return data.todos.find(item => item.title === localStorage.getItem('__subtask_test_title'));
    });
    ...
    await page.waitForFunction(async () => {
      ...
      const todo = todos.find(item => item.title === localStorage.getItem('__subtask_test_title'));
    ...
    const cachedAfterWs = await page.evaluate(async () => {
      const todos = await window.dbGetAll('todos');
      return todos.find(item => item.title === localStorage.getItem('__subtask_test_title'));
    });
    ...
    const cardText = await page.locator('.todo-item').filter({ hasText: TODO_TITLE }).first().innerText();
