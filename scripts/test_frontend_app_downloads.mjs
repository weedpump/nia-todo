...
assert(css.includes('.login-auth-alternatives .login-passkey-btn') && css.includes('flex: 1 1 148px') && css.includes('min-width: min(100%, 148px)') && css.includes('justify-content: center') && css.includes('text-overflow: ellipsis'), 'login auth alternative buttons must use pure CSS flex wrapping, center text, and truncate long provider labels');
