...
function openSecurityDialog({ title, message = '', bodyHtml = '', primaryText = 'Bestätigen', danger = false, onSubmit, onOpen }) {
  ...
  if (!modal || !form || !body || !primary) {
    return Promise.reject(new Error('Sicherheitsdialog ist nicht verfügbar'));
  }

  setText('security-action-title', title);
  setText('security-action-message', message);
  error.textContent = '';
...
export function promptSecurityText({ title, message = '', label = 'Eingabe', name = 'value', value = '', placeholder = '', required = false, autocomplete = 'off', primaryText = 'Weiter' }) {
...
export function promptSecurityPassword({ title, message = '', label = 'Passwort', primaryText = 'Bestätigen' }) {
  return openSecurityDialog({
    title,
    message,
    primaryText,
    bodyHtml: `<label class="security-field"><span>${label}</span><input name="password" type="password" autocomplete="current-password" required></label>`,
    onOpen: (form) => form.querySelector('input')?.focus(),
    onSubmit: (formData) => {
...
export function confirmSecurityAction({ title, message = '', confirmText = 'Bestätigen', danger = false }) {
  return openSecurityDialog({
    title,
    message,
...
export function promptPasskeyDetails() {
  return openSecurityDialog({
    ...
    message: 'Vergib einen Namen und bestätige mit deinem Passwort. Das Passwort bleibt im Formularfeld maskiert.',
    primaryText: 'Passkey erstellen',
    bodyHtml: `
      <label class="security-field"><span>Name</span><input name="name" type="text" value="Passkey" autocomplete="off" maxlength="80" required></label>
      <label class="security-field"><span>Passwort bestätigen</span><input name="password" type="password" autocomplete="current-password" required></label>
    `,
...
export async function performMfaReauth({ authApi, purpose = 'diese Sicherheitsaktion' }) {
  ...
  if (!result) throw new Error('2FA/Reauth abgebrochen');
}
