...
- Refined the Settings modal to follow the shared detail-modal shell with matching sizing, header treatment, flattened section surfaces, round section icons, quieter inner rows/cards, left/right action alignment, secondary API key and 2FA setup actions, inline saved-place entry, muted inactive password/place actions, and an aligned close action.
